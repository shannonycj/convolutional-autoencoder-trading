...
class NetModel:
    ...
    def build_net(self):
        ...
        input_img = Input(shape=(1000, 6, 1))  # adapt this if using `channels_first` image data format
        print("shape of input", K.int_shape(input_img))
        conv_1 = Conv2D(16, conv_shape, activation='relu', padding='same')(input_img)
        print("shape after first conv", K.int_shape(conv_1))
        pool_1 = MaxPooling2D(pooling_shape, padding='same')(conv_1)
        print("shape after first pooling", K.int_shape(pool_1))
        conv_2 = Conv2D(8, conv_shape, activation='relu', padding='same')(pool_1)
        print("shape after second conv", K.int_shape(conv_2))
        
        pool_2 = MaxPooling2D(pooling_shape, padding='same')(conv_2)
        print("shape after second pooling", K.int_shape(pool_2))
        
        conv_3 = Conv2D(8, conv_shape, activation='relu', padding='same')(pool_2)
        print("shape after third conv", K.int_shape(conv_3))
        
        encoded = MaxPooling2D(pooling_shape, padding='same')(conv_3)
        print("shape of encoded", K.int_shape(encoded))
        
        # at this point the representation is (4, 4, 8) i.e. 128-dimensional
        
        up_3 = UpSampling2D(pooling_shape)(encoded)
        print("shape after upsample third pooling", K.int_shape(up_3))
        
        conv_neg_3 = Conv2D(8, conv_shape, activation='relu', padding='same')(up_3)
        print("shape after decode third conv", K.int_shape(conv_neg_3))
        
        up_2 = UpSampling2D(pooling_shape)(conv_neg_3)
        print("shape after upsample second pooling", K.int_shape(up_2))
        
        conv_neg_2 = Conv2D(8, conv_shape, activation='relu', padding='same')(up_2)
        print("shape after decode second conv", K.int_shape(conv_neg_2))
        up_1 = UpSampling2D(pooling_shape)(conv_neg_2)
        print("shape after upsample first pooling", K.int_shape(up_1))
        decoded = Conv2D(1, conv_shape, activation='linear', padding='same')(up_1)
        print("shape after decode first conv", K.int_shape(decoded))
    ...
    def train_classifier(self, model='xgb', n_seach=10):
        if model == 'rf':
